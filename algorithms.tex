...
If applied once, it reduced the argument to $|x| < 1$, then each successive
...
output precision of $1 + \lceil e \log(2)/\log(b) \rceil$.
However the code uses instead $1 + \lceil b x \rceil$, where $y$ is
...
doit := proc(b) local x, y, l, i, e, amax, a, f;
   ...
   lprint("p/q=", e);
   if e<x then
      lprint("p/q smaller than x");
      amax := (denom(l[i+1])-denom(l[i-1]))/denom(e);
      for a to amax do
         f := (numer(l[i-1])+a*numer(e))/(denom(l[i-1])+a*denom(e));
         if f>x then lprint('f'=f,"is greater than x, bound is", denom(f));
            break
         fi
      od
   else
      lprint("p/q greater than x, bound is", denom(e));
   fi
end:
\end{comment}
We get the following bounds (for powers of two there is no error):
\begin{tabular}{cc}
3,  1193652440098 & 21, 500866275153 \\
5,  751072483167 &  22, 1148143737877 \\
6,  524743920263 &  23, 2963487537029 \\
7,  186564318007 &  24, 930741237529 \\
9,  2387304880196 & 25, 751072483167 \\
10, 1074541795081 & 26, 1973399062219 \\
11, 890679595344 &  27, 1193652440098 \\
12, 727742578896 &  28, 319475419871 \\
13, 1553566199646 & 29, 1645653531910 \\
14, 253019868939 &  30, 1190119072066 \\
15, 947578699731 &  31, 2605117443408 \\
17, 628204683310 &  33, 510502880896 \\
18, 3140415720077 & 34, 1611724268329 \\
19, 347220488443 &  35, 820222240621 \\
20, 645428387961 &  36, 693983000597 \\
\end{tabular}
The smallest bound is $e=186564318007$ for $b=7$.
