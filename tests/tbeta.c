...
#define FAILED(p, r, z, w, expected, rnd_mode) mpfr_printf("prec=%d,rnd=%s case failed for z=%Rf,w=%Rf. Got %Rb, expected %Rb\n", p, mpfr_print_rnd_mode (rnd_mode), z, w, r, expected); exit(1)
#define TEST(p, r, z, w, expected) TESTRND(p, r, z, w, expected, MPFR_RNDN)
#define TESTRND(p, r, z, w, expected, rnd_mode) mpfr_beta(r, z, w, rnd_mode); if (is_same(r, expected) != 0) { FAILED(p, r, z, w, expected, rnd_mode); }

int is_same(mpfr_t a, mpfr_t b) {
  int res = 0;
  if (mpfr_cmp(a, b) != 0) {
    res = 1;
  }
...
void test_beta_2exp(mp_prec_t prec, int trials, int spread) {
  mpfr_t r, z, w, expect;
  mpfr_init2(r, prec);
  mpfr_init2(z, prec);
  mpfr_init2(w, prec);
...
void test_beta_hardcoded(mp_prec_t prec) {
  if (prec < 10) {
    prec = 10;
  }
  mpfr_t r, z, w, expect;
...
void test_beta_refl(mp_prec_t prec, mp_rnd_t rnd_mode) {
  mpfr_t r, z, w, expect;
  mpfr_init2(z, prec);
  mpfr_init2(w, prec);
  mpfr_init2(r, prec);
