...
main (void)
{
  ...
  for(pr = MPFR_PREC_MIN ; pr < 500 ; pr++)
    {
      mpfr_init2 (a, pr);
      if (!mpfr_check(a)) PRINT_ERROR ("for init");
      /* Check special cases */
      MPFR_SET_NAN(a);
      if (!mpfr_check(a)) PRINT_ERROR ("for nan");
      MPFR_SET_POS(a);
      MPFR_SET_INF(a);
      if (!mpfr_check(a)) PRINT_ERROR ("for inf");
      MPFR_SET_ZERO(a);
      if (!mpfr_check(a)) PRINT_ERROR ("for zero");
      MPFR_EXP (a) = MPFR_EXP_MIN;
      if (mpfr_check(a))  PRINT_ERROR ("for EXP = MPFR_EXP_MIN");
      /* Check var */
      mpfr_set_ui(a, 2, MPFR_RNDN);
      if (!mpfr_check(a)) PRINT_ERROR ("for set_ui");
      mpfr_clear_overflow();
      max = 1000; /* Allows max 2^1000 bits for the exponent */
      while ((!mpfr_overflow_p()) && (max>0))
        {
          mpfr_mul(a, a, a, MPFR_RNDN);
          if (!mpfr_check(a)) PRINT_ERROR ("for mul");
          max--;
        }
      if (max==0) PRINT_ERROR ("can't reach overflow");
      mpfr_set_ui(a, 2137, MPFR_RNDN);
      /* Corrupt a and check for it */
      MPFR_SIGN(a) = 2;
      if (mpfr_check(a))  PRINT_ERROR ("sgn");
      MPFR_SET_POS(a);
      /* Check prec */
      ...
      if ((mpfr_prec_t) 0 - 1 > 0)
        {
          ...
          if (mpfr_check(a))  PRINT_ERROR ("precmax");
        }
#endif
      MPFR_PREC(a) = pr;
      if (!mpfr_check(a)) PRINT_ERROR ("prec");
      /* Check exponent */
      MPFR_EXP(a) = MPFR_EXP_INVALID;
      if (mpfr_check(a))  PRINT_ERROR ("exp invalid");
      MPFR_EXP(a) = -MPFR_EXP_INVALID;
      if (mpfr_check(a))  PRINT_ERROR ("-exp invalid");
      MPFR_EXP(a) = 0;
      ...
      if ((pr % GMP_NUMB_BITS) != 0)
        {
          ...
          if (mpfr_check(a))  PRINT_ERROR ("last bits non 0");
        }
      MPFR_MANT(a)[0] = tmp;
      ...
      /* Final */
      mpfr_set_ui(a, 2137, MPFR_RNDN);
      if (!mpfr_check(a)) PRINT_ERROR ("after last set");
      mpfr_clear (a);
      if (mpfr_check(a))  PRINT_ERROR ("after clear");
    }
  tests_end_mpfr ();
  return 0;
}
