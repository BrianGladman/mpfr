...
rounding modes. Since $x \in [2^{e-1},2^e[$ and both $2^{e-1}$ and $2^e$
are exactly representable, $y \in [2^{e-1},2^e]$.
Thus $\ulp(x) \leq \ulp(y)$, which completes the proof.
\end{proof}
...
$u := \circ(x)$. Then, assuming that no underflows nor overflows occur,
$\frac{1}{2} |u| < |x| < 2 |u|$.
\end{Rule}
\begin{proof}
