...
\subsubsection{Case $s$ close to $0$}

First, let us recall that the subcase $s = 0$ has already been taken
into account as a special input.

In theory, this case could be handled by the reflection formula
(\textsection\ref{zeta:reflection}), but as the term $\zeta(1-s)$ is
close to the pole of $\zeta$ at $1$, it would be slow (and could even
yield an internal overflow for tiny $s$), while the correctly rounded
result can be determined very quickly when $s$ is close enough to $0$.
We have around $0$:
...
(WORK IN PROGRESS) % To be removed when complete and correctly implemented.
