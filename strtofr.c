  ...
  for (;;)
    {
      ...
      /* and to ysize_bits bits */
      y = (mp_limb_t*) MPFR_TMP_ALLOC ((2 * ysize + 1) * sizeof (mp_limb_t));
      y += ysize; /* y has (ysize+1) allocated limbs */

      /* pstr_size is the number of characters we want to read in pstr->mant
         have ~ysize limbs (again, as long as we guarantee correct rounding,
         we don't need to get exactly ysize limbs).
         We must have (2^(BITS_PER_MP_LIMB))^ysize ~= base^pstr_size
         aka pstr_size ~ ysize*BITS_PER_MP_LIMB/log2(base)
